...
for p in powers_of_2:
    duration = 2**p
    f_array = jnp.arange(0, 1.0/(2*cadance), 1.0/duration)
    ...
    def waveform_ripple(theta):
        return gen_IMRPhenomD_hphc(f_array, theta, f_array[0])

    # jit ahead of time counting
...
with open('time_consuming_gpu_ripple.json', 'w') as f:
